...
| `4_hand_off`  | 완료된 프로젝트 보관. status, completed 메타데이터 유지                |
...
| `/hand-off` | 프로젝트 목록에서 선택 → complete 처리 + hand_off로 이동 |

## 프로젝트 상태 흐름

```
discovery(기획중) → in-progress(진행중) → complete(완료) → hand_off 이동
```
